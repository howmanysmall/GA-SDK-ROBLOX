...
--[[**
	"Overengineered" `delay` reimplementation that also allows passing parameters. This should take significantly less time to execute than the original.
...
--[[**
	A recreation of `spawn`, delay and all. This should run better than the original `spawn` in addition to not using a legacy scheduler and passing parameters to be called.
...
--[[**
	...
	@param [t:function] Function The function you are calling.
	@param [t:...?] ... The optional arguments to call the function with.
	@returns [t:boolean,...?] Whether or not the call was successful and the returned values.
**--]]
...
--[[**
	Spawns the passed function with a delay using RunService.Heartbeat. This keeps the traceback as well, and will throw an error if the function errors.
	@param [t:function] Function The function you are calling.
	@param [t:...?] ... The optional arguments to call the function with.
	@returns [t:void]
**--]]
function Scheduler.HeartbeatSpawn(Function, ...)
	local Length = select("#", ...)
	if Length > 0 then
		local Arguments = {...}
